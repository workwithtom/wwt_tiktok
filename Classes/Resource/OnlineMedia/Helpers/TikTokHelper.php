...
namespace WorkWithTom\WwtTiktok\Resource\OnlineMedia\Helpers;
...
{
    private $tikTokEmbeddedUrl = 'https://www.tiktok.com/oembed?url=%s';
    /**
     * Get public url
     *
     * @param File $file
    ...
    public function getPreviewImage(File $file)
    {
        ...
        if (!file_exists($temporaryFileName)) {
            $previewImage = GeneralUtility::getUrl(
                sprintf($this->tikTokEmbeddedUrl, $videoId)
            );
            if ($previewImage !== false) {
                $previewImage = json_decode($previewImage);
                if ($previewImage) {
                    file_put_contents($temporaryFileName, GeneralUtility::getUrl($previewImage->thumbnail_url));
    ...
    public function getMetaData(File $file)
    {
        ...
        if ($oEmbed) {
            $metadata['width'] = (int)$oEmbed['thumbnail_width'];
            $metadata['height'] = (int)$oEmbed['thumbnail_height'];
            if (empty($file->getProperty('title'))) {
                $metadata['title'] = $this->remove_emoji(strip_tags($oEmbed['title']));
            }
